...
class TestCycleTelemetry(unittest.TestCase):
    def test_shutdown_failure_still_closes_phases_and_telemetry(self):
        ...
        agent._cycle_identity = {"nv.nvrx.cycle.index": 3}
        agent._close_telemetry_cycle.side_effect = (
            lambda attributes=None: launcher.LocalElasticAgent._close_telemetry_cycle(
                agent, attributes
            )
        )
        with (
        ...
        self.assertIsNone(agent._cycle_identity)
        ...
        with (
            ...
            for round_number in (0, 3):
                with self.subTest(round_number=round_number):
                    ...
                    identity = dict(agent._cycle_identity)
                    ...
                    with patch.object(
                        ...
                        side_effect=AssertionError("worker launch re-derived cycle identity"),
                    ):
                    ...
                    self.assertEqual(
                        attrs["nv.nvrx.cycle.index"], str(identity["nv.nvrx.cycle.index"])
                    )
                    self.assertEqual(env["TORCHELASTIC_RESTART_COUNT"], str(round_number))
                    launcher.LocalElasticAgent._close_telemetry_cycle(agent)
                    self.assertIsNone(agent._cycle_identity)
                    with self.assertRaisesRegex(RuntimeError, "active telemetry cycle identity"):
                        launcher.LocalElasticAgent._start_workers(agent, group)

            with patch.object(telemetry, "_AVAILABLE", False):
                launcher.LocalElasticAgent._open_telemetry_cycle(agent, 4)
                self.assertEqual(agent._cycle_identity, {"nv.nvrx.cycle.index": 4})
                launcher.LocalElasticAgent._start_workers(agent, group)
                env = start.call_args.kwargs["envs"][0]
